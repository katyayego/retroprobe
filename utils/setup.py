...
def setup_model(lm_name, dep_dim, parser_type='depprobe', state_dict=None, emb_layers=None, emb_cache=None):
	...
	elif parser_type == 'retroprobe':
		assert len(emb_layers) == 2, f"[Error] RetroProbe requires two embedding layers, received {len(emb_layers)}."
		parser = RetroProbe(
			emb_model=emb_model,
			dep_dim=dep_dim,
			dep_rels=UD_RELATION_TYPES
...
def setup_criterion(parser_type='depprobe'):
	...
	# use depprobe loss
	elif parser_type == 'depprobe' or parser_type == 'retroprobe':
...
# 		f"LabelLoss: {np.round(stats[f'{mode}/loss_label'][-1], 4) if f'{mode}/loss_label' in stats else 'None'}, "
# # 		f"Loss: {stats[f'{mode}/loss'][-1]:.4f}"
#         f"Loss: {stats[f'{mode}/loss'][-1] if f'{mode}/loss' in stats else 'None'}"
